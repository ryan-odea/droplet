...
class CombineMode(Enum):
    ...
    AND = "and"     # Intersection - only where ripples overlap (white only where collision)
...
class BooleanRipple:
    ...
    def add_droplet(self, x: float, y: float, intensity: int = 2, 
                    speed: float = 1.0, max_radius: float = None):
        """
    ...
    def _compute_ripple(self, droplet: Droplet) -> np.ndarray:
        ...
        distances = np.sqrt(
            (self.x_coords - droplet.x) ** 2 + 
            (self.y_coords - droplet.y) ** 2
        )
...
def create_boolean_animation(size: int = 100, 
